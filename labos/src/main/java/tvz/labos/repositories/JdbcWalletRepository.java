...
public class JdbcWalletRepository implements WalletRepository{
     ...
     public JdbcWalletRepository(JdbcTemplate jdbc) {
         this.jdbc = jdbc;
         this.walletInserter = new SimpleJdbcInsert(this.jdbc).withTableName("wallets").usingGeneratedKeyColumns("id");
     }

     @Override
     public Iterable<Wallet> findAll() {
         try {
             return jdbc.query("select id, currentAmount, name, owner from wallets", this::mapRowToWallet);
     ...
     public Wallet findOne(Long id) {
         try {
             return jdbc.queryForObject("select id, currentAmount, name, owner from wallets where id = ?", this::mapRowToWallet, id);
         } catch (EmptyResultDataAccessException ex) {
             logger.info("Finding by id returned 0 results, returning null instead");
             return null;
         }
     ...
     public Wallet findOneByOwner(String owner) {
         try {
             return jdbc.queryForObject("select id, currentAmount, name, owner from wallets where owner = ?", this::mapRowToWallet, owner);
         } catch (EmptyResultDataAccessException ex) {
